...
class Glm4MoeModelToolParser(ToolParser):
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
                ...
                for tool in self.tool_list:
                    ...
                    if function.name == tool_name:
                        ...
                        if function.parameters is None:
                            return False
                        prop = function.parameters.properties.get(arg_name)
                        if prop is None:
                            return True
        ...
        matched_tool_calls = self.func_call_regex.findall(text)
        logger.debug("model_output: %s", text)
        try:
            function_calls = []
            for match in matched_tool_calls:
                ...
                for key, value in pairs:
                    arg_key = key.strip()
                    arg_val = value.strip()
                    if not _is_string_type(tc_name, arg_key):
            ...
            if len(function_calls) > 0:
                content = text[: text.find(self.tool_calls_start_token)]
                return content, function_calls
            return text, []
